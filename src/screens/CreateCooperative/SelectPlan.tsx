  ...
  const handleSelect = (planId: string) => {
    ...
    if (plan.custom) {
      ...
      window.location.href =
        "mailto:support@cooperative.com?subject=Custom Plan for " +
        (tenant?.name || "New Coop");
      return;
    }

    alert(
      `You've chosen the ${plan.name} plan!${
        billing === "yearly" ? " You saved 20% with annual billing!" : ""
      }`
    );

    // Navigate to dashboard with state
    console.log("Selected plan:", { ...plan, billingCycle: billing });
  };
        ...
        <div className="inline-flex gap-1 p-1 bg-gray-100 rounded-lg border border-gray-200">
          {(["monthly", "quarterly", "yearly"] as BillingCycle[]).map((cycle) => (
            <button
              ...
              className={`relative px-4 py-2 rounded-md font-medium text-sm transition-all duration-300 flex items-center gap-2 whitespace-nowrap ${
                billing === cycle
