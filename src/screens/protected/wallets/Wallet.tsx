...
export default function WalletPage() {
  ...
  const {
    ...
    queryFn: () =>
      apiClient.get("/api/platform/wallet").then((res) => res.data),
  });
  ...
  const {
    ...
    queryFn: () =>
      apiClient.get("/api/platform/transactions").then((res) => res),
  });
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
        ...
        <Button
          onClick={handleAutoSweepSettings}
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-600">
              {walletLoading
                ? "—"
                : formatCurrency(wallet?.unsettled_balance || 0)}
            </div>
        ...
        <Card className="border-2 border-primary/20">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-primary">
              {walletLoading
                ? "—"
                : formatCurrency(wallet?.platform_balance || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {walletLoading
                ? "—"
                : formatCurrency(
                    (wallet?.total_platform_deposits || 0) +
                      (wallet?.total_unsettled_deposits || 0)
                  )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {walletLoading
                ? "—"
                : formatCurrency(wallet?.total_platform_withdrawals || 0)}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <p className="font-semibold text-lg">
                  {walletLoading
                    ? "—"
                    : wallet?.main_virtual_bank || "Not configured"}
                </p>
                {!walletLoading && wallet?.main_virtual_bank && (
                  <Button
                    ...
                    className="h-8 w-8"
                    onClick={() =>
                      copyToClipboard(wallet.main_virtual_bank, "Bank name")
                    }
                  >
              ...
              <div className="flex items-center gap-2">
                ...
                {!walletLoading && wallet?.nomba_account_ref && (
                  <Button
                    ...
                    className="h-8 w-8"
                    onClick={() =>
                      copyToClipboard(wallet.nomba_account_ref, "Reference")
                    }
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            All platform deposits, settlements, and withdrawals
          </CardDescription>
        </CardHeader>
        <CardContent>
          {txLoading ? (
